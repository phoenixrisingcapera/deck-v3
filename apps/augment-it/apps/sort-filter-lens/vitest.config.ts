...
export default defineConfig({
  ...
  resolve: {
    ...
    // single assertion runs. corpora-curator never hit it because its tests are
    // logic-only. Copied from packages/shared-ui/vitest.config.ts.
    conditions: ['browser'],
    alias: [
      ...
      // data. The stub below throws on every write verb so the same mistake
      // fails loudly instead of succeeding against production.
      // EXACT match, not a prefix: a bare string key here would also rewrite
