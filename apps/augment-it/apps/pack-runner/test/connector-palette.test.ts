/**
 * pack-runner / ConnectorPalette — THE KEYBOARD, NOT THE PIXELS.
 *
 * DELIBERATELY THE SAME ASSERTIONS AS
 * apps/response-reviewer/test/connector-palette.test.ts. These two palettes are
 * a declared converged twin that had already diverged before this migration,
 * and they answered the Selector question identically — so the twin is asserted
 * against ONE contract rather than two similar ones. The day one of these files
 * needs an assertion the other does not is the day DESIGN.md should stop calling
 * them identical.
 *
