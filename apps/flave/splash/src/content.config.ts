...
// LFM publishes to JSR/npm, but the splash schema must still tolerate every
// frontmatter shape we've ever authored in changelog/ and context-v/. The
// philosophy from the README — "let content creators be content creators" —
// applies to the splash itself: lenient in, structured out.
...
interface LocalLoaderOptions {
  ...
  dir: string;
  /** Logical name — used in id and log lines. */
  collectionName: string;
  /** Provenance tag stamped onto every entry. */
  provenance: string;
}
...
const featureHighlights = defineCollection({
  loader: glob({ pattern: '*.md', base: './src/content/feature-highlights' }),
  schema: z
    .object({
      title: lenientString,
      lede: lenientString,
