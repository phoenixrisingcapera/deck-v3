...
export async function POST({ params, fetch, cookies }) {
  ...
  const response = await fetch(`${backendUrl}/api/decks/${params.deckId}/due-diligence/extract-claims`, {
    method: 'POST',
    headers: requireBackendAuthHeaders(cookies)
  });
  const result = await response.json().catch(() => null);
  if (!response.ok) {
    throw error(response.status, extractErrorMessage(result, 'Due diligence claim extraction failed.'));
  }
