...
export async function POST({ params, fetch, cookies }) {
  ...
  const response = await fetch(`${backendUrl}/api/decks/${params.deckId}/due-diligence/analyze-risks`, {
    method: 'POST',
    headers: requireBackendAuthHeaders(cookies)
  });
  const result = await response.json().catch(() => null);
  if (!response.ok) {
    throw error(response.status, extractErrorMessage(result, 'Due diligence risk analysis failed.'));
  }
