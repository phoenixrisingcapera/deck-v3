...
export async function GET({ params, fetch, cookies }) {
  const response = await fetch(`${requireBackendUrl()}${deckProductApiPath(`/decks/${params.deckId}/media/${params.mediaId}/thumbnail`)}`, { headers: requireBackendAuthHeaders(cookies) });
  if (!response.ok || !response.body) throw error(response.status, 'Media thumbnail could not be loaded.');
