...
authors:
- Michael Staton
image_prompt: on the left and right side of the image, robots are at desks in rows
  typing on a typewriter furiously.  The desks are facing each other.  The typewriters
  are writing on one continuous roll of paper, so there is a scramble of paper all
  over the floor.  On the extreme right and left a `line foreman` is coaching and
  motivating the typists.
