...
description: 'Answer the second foundational VC question — how fast can it get that
  big? — by stress-testing a company''s current MoM/YoY growth at its actual unit
  of sale across four scenarios: sustain, improve, plateau, reduce. Use whenever asked
  "how long until", "how fast can they grow into this", "is this reachable in the
  fund''s horizon", or "what if growth slows"; whenever a market-sizing number needs
  a time axis before it means anything; whenever building or reviewing the growth
  or timeline section of an investment memo. Consumes the penetration grid produced
  by market-capture-analysis — run that first, or there is nothing to stress-test.
  Encodes the four scenarios, the sensitivity discipline that keeps a scenario from
  becoming a forecast, and the failure modes that make growth math flattering.'
title: Timeline Scenario Analysis
lede: 'Answers the second of the two foundational VC questions — *how fast can it
  get that big?* — by stress-testing the company''s current MoM/YoY growth at its
  actual unit of sale across four scenarios: sustain, improve, plateau, reduce. Operates
  on the penetration grid produced by the sibling skill [[market-capture-analysis]];
...
source_path: "context-v/agent-skills/timeline-scenario-analysis/SKILL.md"
---
