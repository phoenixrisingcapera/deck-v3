...
lede: 'Neo4j has to be running. The model alias in graphiti-core''s own default doesn''t
  resolve. And `publish: false` means one thing to the changelog ingester and nothing
  at all to the context-v one. Short, sharp, learned the hard way.'
summary: Corrections for agents working with the kit's two retrieval layers — Chroma
  for semantic search, Graphiti-on-Neo4j for the temporal graph. Covers the query
  discipline, the frontmatter keys each layer actually reads, the three load-bearing
  initialisation gotchas in graphiti_clients.py, and the publish-gate asymmetry between
  ingesters. Read before querying either layer, before running any ingest, and before
  changing frontmatter that a retrieval layer consumes.
publish: true
